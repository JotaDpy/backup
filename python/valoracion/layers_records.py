...
def buscar_valuation(product_id):
    # Nota el 'return', si no la función no devuelve nada
    producto = env['product.product'].browse(product_id)
    return producto
...
producto = buscar_valuation(3281)
valoraciones = producto.stock_valuation_layer_ids
campos_usados = [
    'id', 'display_name',
...
# Cambiar las columnas que se muestran en el csv
print(producto.categ_id.name)
print(valoraciones)
